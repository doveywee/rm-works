...
void main() {
  ...
  float t = uTime * 0.35;
  float n1 = snoise(position * 1.4 + vec3(0.0, t, 0.0));
  float n2 = snoise(position * 3.1 - vec3(t * 0.6, 0.0, t * 0.4)) * 0.5;
  float mouseEnergy = 0.4 + length(uMouse) * 0.9;
  float disp = (n1 + n2) * uAmp * mouseEnergy;
  vDisp = disp;
...
void main() {
  ...
  // Iridescence driven by view angle + displacement, animated over time
  float iri = fresnel + vDisp * 1.5 + uTime * 0.05;
  vec3 irid = palette(iri);

  vec3 base = mix(uColorA, uColorB, smoothstep(-0.3, 0.3, vDisp));
  vec3 color = mix(base, irid, fresnel * 0.9);

  // inner glow + rim light
  color += irid * fresnel * 0.6;
